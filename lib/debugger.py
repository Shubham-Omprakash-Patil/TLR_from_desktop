...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.names = wood_class_name
    elif dataset == 'magna':
      self.names = magna_class_name
  ...
  def overlay_seg_img(self, img_seg, img_id='default'):
    h, w, c = self.imgs[img_id].shape
    img_seg_resized = cv2.resize(img_seg.astype(np.uint8), (w, h), interpolation = cv2.INTER_AREA)
    segmap = SegmentationMapsOnImage(img_seg_resized, shape=self.imgs[img_id].shape)

    self.imgs[img_id] = segmap.draw_on_image(self.imgs[img_id])[0]
...
wood_class_name = [
  'vehicles', 'pedestrians', 'bicycle', 'traffic lights', 'traffic sign'
]

magna_class_name = [ 'NA', 'ts', 'tl']
