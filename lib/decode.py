...
def ctdet_decode_tl(heat, wh, cls_nb, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()
    ...
    cls_nb = _transpose_and_gather_feat(cls_nb, inds)
    cls_nb = cls_nb.view(batch, K, 2).float()    
    scores = scores.view(batch, K, 1)
    bboxes = torch.cat([xs - wh[..., 0:1] / 2, 
                        ys - wh[..., 1:2] / 2,
                        xs + wh[..., 0:1] / 2, 
    ...
    detections = torch.cat([bboxes, scores, cls_nb], dim=2)    
    return detections
